      ...
      if (!sentDocumentIds.has(docId)) {
        ...
        if (nombre && edadic && pais && socialClass && historyPj && oocage && steamurl && nomeDiscord && status) {
          const embed = new MessageEmbed()
            ...
            .setFooter(`🔸𝔻♞ℂ 𝕎ℍ𝕀𝕋𝔼𝕃𝕀𝕊𝕋 𝕊𝕐𝕊𝕋𝔼𝕄🔸 - ${getCurrentDateTime()}`, 'https://example.com/logo.png');
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId === 'approve_whitelist') {
    ...
    await interaction.deferReply();
    await interaction.followUp("Por favor, inserta una razón en el siguiente mensaje.");

    pendingRejections.set(interaction.user.id, {
      interaction,
      docId: interaction.message.embeds[0].fields.find(field => field.name === 'Nombre de Discord').value,
...
client.on('messageCreate', async message => {
  if (pendingRejections.has(message.author.id)) {
    const { interaction, docId, channelId } = pendingRejections.get(message.author.id);
    if (message.channel.id === channelId) {
      const rejectionReason = message.content;
      const guildMember = await interaction.guild.members.fetch({ query: docId, limit: 1 });
      if (!guildMember || !guildMember.first()) {
        console.error(`No se pudo encontrar al usuario con el nombre de Discord: ${docId}`);
        return interaction.editReply('Error: No se pudo encontrar al usuario correspondiente.');
      ...
      const rejectionMessage = `Tu solicitud ha sido rechazada. Razón: ${rejectionReason}`;
      ...
      try {
        ...
        querySnapshot.forEach(async doc => {
          ...
          await doc.ref.update({ reason: rejectionReason });
        });
      } catch (error) {
        console.error('Error al actualizar el estado en la base de datos:', error);
      }
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        querySnapshot.forEach(doc => {
          const timeStampDocumento = doc.data().timeStamp;
          const diferencia = Math.abs(timeStampDocumento - fechaActual);
      ...
      if (docMasCercano) {
        const { nombre, edadic, pais, socialClass, historyPj, oocage, steamurl, nomeDiscord, status, reason } = docMasCercano.data();
        ...
        const embed = new MessageEmbed()
          ...
          if (reason) {
              embed.addField('Razón de rechazo', reason);
          }
